...
#define MAX_MESSAGE_LENGTH 2   // Chunked messages size //can only transmit 
...
void transmitMessageChunks(const String& message) {
    ...
    for (int chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
        ...
        // Print the computed tag
        Serial.print(F("Computed Tag: "));
        for (int i = 0; i < sizeof(tag); i++) {
            // Print each byte of the tag in hex format
            if (tag[i] < 0x10) {
                Serial.print(F("0")); // Print leading zero for single hex digits
            }
            Serial.print(tag[i], HEX); // Print byte as hex
        }
            ...
            if (inputText.equalsIgnoreCase("clear")) {
                ...
                if (compareHashes(hashResult, storedHash, HASH_SIZE)) {
                    ...
                    if (DEST_NODE >= 2 && DEST_NODE <= 99) {
                        ...
                        Serial.println(F("Please enter a message to encrypt (maximum 50 characters including spaces):"));
